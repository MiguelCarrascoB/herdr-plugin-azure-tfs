...
class TfsClient {
  ...
  async request(method, pathname, body, query) {
    const response = await this.fetch(this.url(pathname, query), { method, headers: { Authorization: authHeader(this.c.pat), Accept: 'application/json', ...(body ? { 'Content-Type': Array.isArray(body) ? 'application/json-patch+json' : 'application/json' } : {}) }, body: body === undefined ? undefined : JSON.stringify(body) });
