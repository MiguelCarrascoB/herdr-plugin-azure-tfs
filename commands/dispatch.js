...
(async () => {
  try {
    const ctx = context(); const id = Number(process.argv[2] || ctx.workItemId || ctx.id); if (!Number.isInteger(id)) throw new Error('Usage: dispatch <work-item-id>');
    ...
    const agent = process.env.TFS_AGENT_COMMAND || ctx.agent || 'opencode';
